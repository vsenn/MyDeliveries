//
// PackagesTypeMasterViewController.swift
// MyDeliveries
//
...
class PackagesTypeMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var deliveryService: DeliveryService<OnlineODataProvider> {
        return self.appDelegate.deliveryService
    }

    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([PackagesType]?, Error?) -> Void) -> Void)?
    private var entities: [PackagesType] = [PackagesType]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let packagestype = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "PackageID", value: "\(packagestype.packageID!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! PackagesTypeDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].packageID ?? ""
...
extension PackagesTypeMasterViewController: EntitySetUpdaterDelegate {
